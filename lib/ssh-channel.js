...
require('util').inherits(SSHChannel, require('events').EventEmitter);
...
SSHChannel.prototype._forwardRequestToChannel = function (payload, cb) {
	...
	channel.processRequest(this, req, want_reply, payload, cb);
};
...
SSHChannel.prototype._forwardDataToChannel = function (payload, cb) {
	...
	this.emit('data', channel.buildRequest(data), buildResponseObject(channel.client_channel, cb));
};
...
Channel.prototype.buildRequest = function (data) {
	var request = {
		channel: {
			type: this.type,
			window_size: this.window_size,
			environmental_variables: this._env,
			isShell: this._shell
		},
		user: this.user,
	};
	...
	if(this._terminal) { 
		request.terminal = {
			environment: this._terminal.environment,
			columns: this._terminal.columns,
			rows: this._terminal.rows,
			width: this._terminal.pixel_width,
			height: this._terminal.pixel_height,
			mode: this._terminal.mode
		};
	}
...
Channel.prototype.processRequest = function (ssh, req, want_reply, payload, cb) {
	switch(req) {
	case 'pty-req':
		...
		ssh.emit('terminal', this.buildRequest(), buildResponseObject(this.client_channel, cb));
		break;
	case 'shell':
		this._enableShell();
		break;
