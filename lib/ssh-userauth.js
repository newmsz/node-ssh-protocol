...
require('util').inherits(SSHUserAuth, require('events').EventEmitter);
...
SSHUserAuth.prototype.authenticate = function (packet, cb) {
	...
	if(!this.userAuthenticated) {
		...
		this._verifyAuthenticationRequest(payload, _.bind(function (err, msgno, output) {
...
SSHUserAuth.prototype._verifyAuthenticationRequest = function (payload, cb) {
	...
	if(method !== 'none' && (AVAILABLE_METHODS.indexOf(method) < 0))
		method = 'none';
	...
	case 'none':
		var mtd = [];
		if(AVAILABLE_METHODS.length === 0) mtd.push('none');
		else mtd = AVAILABLE_METHODS;
		
		var partial_success = new Buffer(1),
			output = SSHPacket.createNameList(mtd);
		partial_success.writeUInt8(0, 0);
		output = Buffer.concat([output, partial_success], output.length + 1); 
		return cb(null, MESSAGE_NUMBER.SSH_MSG_USERAUTH_FAILURE, output);
		break;	
	case 'publickey':
		...
		if(!has_signature) {
			...
			return cb(null, MESSAGE_NUMBER.SSH_MSG_USERAUTH_PK_OK, Buffer.concat([alg_name, blob], alg_name.length + blob.length));
		} 
	...
	case 'password':
		if(payload.readUInt8(0) === 0) { // not PASSWD_CHANGEREQ
			...
			this.emit('authentication', {
				...
				fail: function () {
					var partial_success = new Buffer(1),
						methods = SSHPacket.createNameList(AVAILABLE_METHODS);
					partial_success.writeUInt8(0, 0);
					return cb(null, MESSAGE_NUMBER.SSH_MSG_USERAUTH_FAILURE, Buffer.concat([methods, partial_success], methods.length + 1));
				},
			});
		} else {
		}
...
function setAuthenticationMethod (methods) {
	for(var i=0; i<methods.length; i++) {
		switch(methods[i]) {
		case 'password': AVAILABLE_METHODS.push('password'); break;
		default: throw new Error('unknown authentication method: ' + methods[i]);
		}
	}
}
