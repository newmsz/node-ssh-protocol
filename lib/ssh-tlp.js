...
function TLP (conn, al) {
	...
	this._user.onUserAuth(_.bind(function (request, response) {
		this._al.emit('userauth', request, response);
	}, this));
	this._channel = new SSHChannel.SSHChannel();
	this._channel.onData(_.bind(function (request, response) {
		this._al.emit('data', request, response);
	}, this));
}
...
TLP.prototype._process = function () {
	console.log(this._state, this._packet.length);
	switch(this._state) {
